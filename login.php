...
if (isset($_POST['login'])) {
    ...
    / Regular expression pattern for password: At least one uppercase letter, one lowercase letter, and one digit
    $password_pattern = '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/';

    // Check if the password matches the pattern
    if (!preg_match($password_pattern, $password)) {
        echo "Invalid password format! Password must contain at least one uppercase letter, one lowercase letter, and one digit.";
        exit();
    }
